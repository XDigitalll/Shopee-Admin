...
import { markDeliveryStarted } from "@/lib/admin/delivery-meta-store";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    response = await fetchBackend(request, `/admin/delivery/active/${id}/start`, {
      method: "POST",
    });
  ...
  if (!response.ok) {
    return jsonError(
      await readBackendMessage(response, "Nao foi possivel iniciar a entrega."),
      response.status
    );
  }

  const meta = markDeliveryStarted(Number(id));
  return NextResponse.json({ ok: true, startedAt: meta.startedAt });
}
