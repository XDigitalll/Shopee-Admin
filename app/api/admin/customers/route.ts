...
function mapCustomer(item: Partial<AdminCustomer>): AdminCustomer {
  return {
    ...
    whatsappPhoneNumber: item.whatsappPhoneNumber ?? null,
    whatsappVerified: Boolean(item.whatsappVerified ?? false),
    whatsappOptIn: Boolean(item.whatsappOptIn ?? false),
    whatsappLinkedAt: item.whatsappLinkedAt ?? null,
    lastWhatsappInteractionAt: item.lastWhatsappInteractionAt ?? null,
    preferredContactChannel: (item.preferredContactChannel as AdminCustomer["preferredContactChannel"]) ?? null,
