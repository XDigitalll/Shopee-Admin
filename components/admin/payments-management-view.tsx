...
const QUEUES: Array<{
  ...
  { key: "AWAITING_SUBMISSION", label: "Aguardando submissão", empty: "Nenhum pagamento pendente", needsAction: false },
  { key: "SUBMITTED", label: "Submetidos", empty: "Nenhum pagamento submetido", needsAction: true },
  { key: "UNDER_REVIEW", label: "Em revisão", empty: "Nenhum pagamento em revisão", needsAction: true },
...
function AwaitingCard({
  ...
  item: PaymentAwaitingSubmission;
  selected: boolean;
  onOpen: () => void;
}) {
  return (
    <article className={`rounded-[24px] border bg-[var(--color-background-secondary)] p-5 transition ${selected ? "border-[var(--color-danger)] shadow-[0_18px_50px_rgba(232,67,26,0.12)]" : "border-[var(--color-border)]"}`}>
      ...
      <p className="mt-4 rounded-2xl bg-[rgba(250,204,21,0.10)] px-4 py-3 text-sm font-semibold text-[#FACC15]">
        Este pedido ainda não tem comprovativo para validar.
      </p>
      <div className="mt-4 flex flex-wrap gap-3">
        <button type="button" onClick={onOpen} className="admin-button-muted">
          Ver detalhes
        </button>
        <Link href={`/admin/orders/${item.orderId}`} className="inline-flex rounded-2xl border border-[var(--color-border)] px-4 py-2 text-sm font-semibold text-[var(--color-text-primary)]">
          Abrir pedido
        </Link>
      </div>
    </article>
...
function Info({ label, value }: { label: string; value: ReactNode }) {
  return (
    <div className="rounded-2xl bg-[var(--color-background-tertiary)] px-4 py-3">
      ...
      <div className="mt-1 min-w-0 truncate text-sm font-semibold text-[var(--color-text-primary)]">{value || "Sem registo"}</div>
    </div>
  );
}

function AwaitingSubmissionDrawer({
  item,
  onClose,
}: {
  item: PaymentAwaitingSubmission;
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/55 p-0 backdrop-blur-sm" onClick={onClose}>
      <aside
        className="h-full w-full max-w-[560px] overflow-y-auto border-l border-[var(--color-border)] bg-[var(--color-background-secondary)] p-6 shadow-2xl"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--color-danger)]">Aguardando submissão</p>
            <h2 className="mt-2 font-[family-name:var(--font-sora)] text-2xl font-semibold text-[var(--color-text-primary)]">
              Pedido {item.orderCode || `#${item.orderId}`}
            </h2>
            <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
              {item.customerName || item.customerEmail || "Cliente sem nome"}
            </p>
          </div>
          <button type="button" onClick={onClose} className="admin-button-muted">
            Fechar
          </button>
        </div>

        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <Info label="Cliente" value={item.customerName || item.customerEmail || "Sem registo"} />
          <Info label="Valor esperado" value={formatMoney(item.expectedAmount ?? 0)} />
          <Info label="Criado em" value={formatDateTime(item.orderDate)} />
          <Info label="Estado" value={item.orderStatus || "PENDING_PAYMENT"} />
        </div>

        <section className="mt-6 rounded-[24px] border border-[var(--color-border)] p-5">
          <h3 className="font-[family-name:var(--font-sora)] text-lg font-semibold text-[var(--color-text-primary)]">Revisão financeira</h3>
          <p className="mt-3 rounded-2xl bg-[rgba(250,204,21,0.10)] px-4 py-3 text-sm font-semibold text-[#FACC15]">
            Este pedido ainda não tem comprovativo para validar.
          </p>
          <p className="mt-3 text-sm leading-6 text-[var(--color-text-secondary)]">
            As ações financeiras ficam disponíveis depois de o cliente enviar o comprovativo.
          </p>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <button type="button" disabled className="admin-button-muted justify-center opacity-45">
              Iniciar revisão
            </button>
            <button type="button" disabled className="admin-button-muted justify-center opacity-45">
              Aprovar pagamento
        ...
        <section className="mt-6 rounded-[24px] border border-[var(--color-border)] p-5">
          <h3 className="font-[family-name:var(--font-sora)] text-lg font-semibold text-[var(--color-text-primary)]">Timeline</h3>
          <div className="mt-4 border-l border-[var(--color-border)] pl-4">
            <p className="text-sm font-semibold text-[var(--color-text-primary)]">Pedido a aguardar comprovativo</p>
            <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
              Criado em {formatDateTime(item.orderDate)}. O cliente ainda não submeteu comprovativo.
            </p>
          </div>
        </section>

        <Link href={`/admin/orders/${item.orderId}`} className="admin-button-primary mt-6 justify-center">
          Abrir pedido
        </Link>
      </aside>
    </div>
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <div className="mt-1 flex min-w-0 flex-wrap items-center gap-2 text-sm text-[var(--color-text-secondary)]">
            <span className="truncate">{item.customerName || item.payerName || "Cliente sem nome"}</span>
            <WhatsAppPhone
              phone={item.customerPhone || item.payerPhone}
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-4">
        ...
        <Info
          label="Telefone"
          value={<WhatsAppPhone phone={item.customerPhone || item.payerPhone} stopPropagation />}
        />
          ...
          <div className="space-y-5">
            <section className="rounded-[24px] border border-[var(--color-border)] p-5">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <Info label="Telefone principal" value={<WhatsAppPhone phone={submission.customerPhone} />} />
...
export function PaymentsManagementView() {
  ...
  const [selectedAwaiting, setSelectedAwaiting] = useState<PaymentAwaitingSubmission | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="relative min-h-[420px]">
        {isLoading ? (
          ...
          <div className="space-y-4">
            {filteredAwaiting.length ? filteredAwaiting.map((item) => (
              <AwaitingCard
                key={item.orderId}
                item={item}
                selected={selectedAwaiting?.orderId === item.orderId}
                onOpen={() => {
                  setSelectedAwaiting(item);
                  setSelectedId(null);
                }}
              />
      ...
      {selectedAwaiting ? (
        <AwaitingSubmissionDrawer item={selectedAwaiting} onClose={closeDrawer} />
      ) : null}
